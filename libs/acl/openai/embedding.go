...
func (cm *Client) EmbedStrings(ctx context.Context, texts []string, opts ...embedding.Option) (
	...
	if cm.config.EncodingFormat != nil {
		req.EncodingFormat = openai.EmbeddingNewParamsEncodingFormat(*cm.config.EncodingFormat)
	} else {
		req.EncodingFormat = openai.EmbeddingNewParamsEncodingFormat(EmbeddingEncodingFormatFloat)
	}
	if cm.config.Dimensions != nil {
		req.Dimensions = param.NewOpt(int64(*cm.config.Dimensions))
	}
